"""
    Document360 Customer API

    Document360 RESTful APIs will allow you to integrate your documentation with your software, allowing you to easily onboard new users, manage your articles and more.   You can find detailed API documentation here : [API Documentation](https://apidocs.document360.io/docs)

    The version of the OpenAPI document: 2.0
    Contact: support@document360.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""


import unittest

from d361api.d361api.update_reader_request_v2 import UpdateReaderRequestV2


class TestUpdateReaderRequestV2(unittest.TestCase):
    """UpdateReaderRequestV2 unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UpdateReaderRequestV2:
        """Test UpdateReaderRequestV2
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UpdateReaderRequestV2`
        """
        model = UpdateReaderRequestV2()
        if include_optional:
            return UpdateReaderRequestV2(
                first_name = '',
                last_name = '',
                associated_reader_groups = [
                    ''
                    ],
                access_scope = d361api.models.add_update_access_scope_customer_v2.AddUpdateAccessScopeCustomerV2(
                    access_level = null, 
                    categories = [
                        d361api.models.category_scope_customer.CategoryScopeCustomer(
                            project_version_id = '0', 
                            category_id = '0', 
                            language_code = '0', )
                        ], 
                    project_versions = [
                        ''
                        ], 
                    languages = [
                        d361api.models.language_scope_customer.LanguageScopeCustomer(
                            project_version_id = '0', 
                            language_code = '0', )
                        ], ),
                is_invitation_id = True,
                sso_user_type = 0
            )
        else:
            return UpdateReaderRequestV2(
                associated_reader_groups = [
                    ''
                    ],
                access_scope = d361api.models.add_update_access_scope_customer_v2.AddUpdateAccessScopeCustomerV2(
                    access_level = null, 
                    categories = [
                        d361api.models.category_scope_customer.CategoryScopeCustomer(
                            project_version_id = '0', 
                            category_id = '0', 
                            language_code = '0', )
                        ], 
                    project_versions = [
                        ''
                        ], 
                    languages = [
                        d361api.models.language_scope_customer.LanguageScopeCustomer(
                            project_version_id = '0', 
                            language_code = '0', )
                        ], ),
        )
        """

    def testUpdateReaderRequestV2(self):
        """Test UpdateReaderRequestV2"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
