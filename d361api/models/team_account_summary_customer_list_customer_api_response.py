...
try:
    ...
    try:
        ...
        TeamAccountSummaryCustomerListCustomerApiResponse = getattr(
            d361api, "TeamAccountSummaryCustomerListCustomerApiResponse", None
        )
        if TeamAccountSummaryCustomerListCustomerApiResponse is None:
            raise ImportError(
                "Could not find TeamAccountSummaryCustomerListCustomerApiResponse in d361api module"
            )
        __all__ = ["TeamAccountSummaryCustomerListCustomerApiResponse"]
        ...
        warnings.warn(
            f"Failed to import TeamAccountSummaryCustomerListCustomerApiResponse: {e}",
            ImportWarning,
        )
        __all__ = []
