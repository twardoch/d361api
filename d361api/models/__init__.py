...
try:
    ...
    for name in dir(d361api):
        ...
        if (
            ...
            and not name.endswith("Api")  # Exclude API classes
            and name not in ["ApiClient", "Configuration", "ApiResponse"]  # Exclude client classes
            and not name.startswith("OpenApi")
            and not name.startswith("Api")  # Exclude exception classes
            and hasattr(obj, "__module__")
            and "d361api.d361api." in obj.__module__
        ):  # Only include model classes
            globals()[name] = obj
    ...
    try:
        from d361api.d361api.api_documentation_import_response_customer import (
