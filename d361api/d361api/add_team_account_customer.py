...
class AddTeamAccountCustomer(BaseModel):
    ...
    email_id: Annotated[str, Field(min_length=1, strict=True)] = Field(
        description="The email address of the team account"
    )
    first_name: Optional[StrictStr] = Field(
        default=None, description="First name of the team account"
    )
    last_name: Optional[StrictStr] = Field(
        default=None, description="Last name of the team account"
    )
    invited_by: Annotated[str, Field(min_length=1, strict=True)] = Field(
        description="The ID of an existing team account. To get the id, please refer GET **/v2/Teams** endpoint"
    )
    is_sso_user: Optional[StrictBool] = Field(
        default=None, description="Set this to true, if you are adding an SSO user"
    )
    scheme_name: Optional[StrictStr] = Field(
        default=None,
    ...
    skip_sso_invitation_email: Optional[StrictBool] = Field(
        default=None,
        description="Set this to true, if you would like to avoid sending an invitation email to the newly added team account.  This property is applicable only when adding a SSO user.",
    )
    associated_portal_role_id: Annotated[str, Field(min_length=1, strict=True)] = Field(
        description="The associated portal role id. To get the list of portal roles refer /teams/roles endpoint."
    )
    content_permissions: List[ContentPermissionCustomer] = Field(
        description="The content level permissions for the newly added team account"
    )
    associated_groups: Optional[List[StrictStr]] = Field(
        default=None,
        ...
        _obj = cls.model_validate(
            {
                ...
                "content_permissions": [
                    ContentPermissionCustomer.from_dict(_item)
                    for _item in obj["content_permissions"]
                ]
                if obj.get("content_permissions") is not None
                else None,
