...
class AddReaderRequestV2(BaseModel):
    ...
    first_name: Optional[StrictStr] = Field(
        default=None, description="The first name of the reader"
    )
    ...
    associated_reader_groups: Optional[List[StrictStr]] = Field(
        default=None, description="List of reader group IDs"
    )
    access_scope: Optional[AddUpdateAccessScopeCustomerV2] = Field(
        default=None,
        description="If access scope is not given, then 0-None access level will be assigned",
    )
    is_sso_user: Optional[StrictBool] = Field(
        default=None, description="Specify if the reader is a Single Sign-On user or not"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.associated_reader_groups is None
            and "associated_reader_groups" in self.model_fields_set
        ):
