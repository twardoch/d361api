...
class CategorySettingsCustomer(BaseModel):
    ...
    description: Optional[StrictStr] = Field(
        default=None, description="SEO description of the category"
    )
    allow_comments: Optional[StrictBool] = Field(
        default=None, description="Allow comments for category"
    )
    show_table_of_contents: Optional[StrictBool] = Field(
        default=None,
        description="`True` - Table of Contents will be enabled   `False` - Table of Contents will be disabled",
    )
    featured_image_url: Optional[StrictStr] = Field(
        default=None, description="URL of the featured image"
    )
    tags: Optional[List[StrictStr]] = Field(
        default=None, description="List of Tags associated to the category"
    )
    status_indicator: Optional[ArticleStatusIndicator] = Field(
        default=None,
        description="Category status in the knowledge base. 0 - None; 1 - New; 2 - Updated; 3 - Custom",
    )
    status_indicator_expiry_date: Optional[datetime] = Field(
        default=None,
        description="The number of days after which the article status will be removed",
    )
    exclude_from_search: Optional[StrictBool] = Field(
        default=None,
        description="**true** - The caetgory will not appear in search results in the knowledge base  **false** - The category will appear in search results in the knowledge base",
    )
    exclude_from_ai_search: Optional[StrictBool] = Field(
        default=None,
        description="**true** - The AI search assistant will not fetch information from this page category;   **false** - The AI search assistant will not exclude this page category while fetching information",
    )
    related_articles: Optional[List[RelatedArticleData]] = Field(
        default=None, description="The list of related article IDs to show in the knowledge base"
    )
    content_type: Optional[ArticleContentType] = Field(
        default=None, description="0 - Markdown content; 1 - HTML content"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.status_indicator_expiry_date is None
            and "status_indicator_expiry_date" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "related_articles": [
                    RelatedArticleData.from_dict(_item) for _item in obj["related_articles"]
                ]
                if obj.get("related_articles") is not None
                else None,
