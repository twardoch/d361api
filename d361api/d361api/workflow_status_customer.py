...
class WorkflowStatusCustomer(BaseModel):
    ...
    read_only: Optional[StrictBool] = Field(
        default=None,
        description="Indicates whether read-only mode is toggled on for the workflow status. <br></br>  • true: workflow status is set to read-only.No edits can be made to the article.No comments can be added to the article. <br></br>  • false: Contributors can edit the article and add comments to the article to provide feedback.",
    )
