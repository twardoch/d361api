...
class Hit(BaseModel):
    ...
    is_hidden: Optional[StrictBool] = Field(
        default=None, description="Indicates if the article is visible on the site"
    )
    ...
    version: Optional[StrictInt] = Field(
        default=None, description="The version number that is currently published"
    )
    ...
    is_category_hidden: Optional[StrictBool] = Field(
        default=None, description="Indicates if category is visible on the site"
    )
    ...
    is_draft: Optional[StrictBool] = Field(
        default=None, description="Indicates if the article is marked as a draft"
    )
    exclude: Optional[StrictBool] = Field(
        default=None,
        description="Indicates if the article is excluded from search results on user website",
    )
    breadcrumb: Optional[StrictStr] = Field(
        default=None, description="The breadcrumb of the article"
    )
    is_category: Optional[StrictBool] = Field(
        default=None,
        description="If the value is True, then the object has to considered as a category, otherwise it is an article",
    )
    attachment_ids: Optional[List[Any]] = Field(
        default=None, description="The IDs of the files attached to the article"
    )
    is_deleted: Optional[StrictBool] = Field(
        default=None,
        description="If the value is True, then it indicates that the article has been deleted",
    )
    is_folder_type_category: Optional[StrictBool] = Field(
        default=None,
        description="If the value is True, then it indicates that the category is a folder type category",
    )
    updated_on_timestamp: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="The last updated timestamp of the article"
    )
    ...
    is_latest_version: Optional[StrictBool] = Field(
        default=None, description="If the value is True, then the article is the latest version"
    )
    contributors: Optional[List[StrictStr]] = Field(
        default=None, description="The IDs of users who have contributed to the article"
    )
    is_shared_article: Optional[StrictInt] = Field(
        default=None,
        description="If the value is True, then it indicates that it is a shared article",
    )
    lang_code: Optional[StrictStr] = Field(
        default=None, description="The language code of the article or category"
    )
    category_unique_id: Optional[StrictStr] = Field(
        default=None,
        description="A unique identifier for the category which is a combination of category ID and the language code",
    )
    unique_id: Optional[StrictStr] = Field(
        default=None,
    ...
    deleted_by: Optional[StrictStr] = Field(
        default=None, description="The ID of the user who deleted the article"
    )
    deleted_at: Optional[StrictStr] = Field(
        default=None, description="The timestamp when the article was deleted"
    )
    is_git_hub_entity: Optional[StrictBool] = Field(
        default=None,
        description="If the value is True, then it indicates that the article is synced from GitHub",
    )
    original_article_id: Optional[StrictStr] = Field(
        default=None, description="The original article ID. Applicable only for shared articles."
    )
    object_id: Optional[StrictStr] = Field(default=None, description="algolia search object ID")
    snippet_result: Optional[Snippetresult] = Field(
        default=None,
        description="Markup text with occurrences highlighted.",
        alias="_snippet_result",
    )
    highlight_result: Optional[Highlightresult] = Field(
        default=None, description="Highlighted attributes", alias="_highlight_result"
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # and model_fields_set contains the field
        if (
            self.is_folder_type_category is None
            and "is_folder_type_category" in self.model_fields_set
        ):
        ...
        _obj = cls.model_validate(
            {
                ...
                "_snippet_result": Snippetresult.from_dict(obj["_snippet_result"])
                if obj.get("_snippet_result") is not None
                else None,
                "_highlight_result": Highlightresult.from_dict(obj["_highlight_result"])
                if obj.get("_highlight_result") is not None
                else None,
            }
        )
        return _obj
